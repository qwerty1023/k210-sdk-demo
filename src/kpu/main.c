...
#define PLL1_OUTPUT_FREQ 600000000UL
...
int main(void)
{
    ...
    volatile uint8_t fileNameCounter = 0;
    ...
    g_lcd_gram0 = (uint32_t *)iomem_malloc(IMG_W * IMG_H * 2);
    g_lcd_gram1 = (uint32_t *)iomem_malloc(IMG_W * IMG_H * 2);
    g_ai_buf = (uint8_t *)iomem_malloc(IMG_W * IMG_H * 3);
    
    g_display_buf = (uint32_t *)iomem_malloc(IMG_W * IMG_H * 2);
...
//    dvp_set_image_format(DVP_CFG_YUV_FORMAT);
    ...
    dvp_set_ai_addr((uint32_t)g_ai_buf, (uint32_t)(g_ai_buf + IMG_W * IMG_H), (uint32_t)(g_ai_buf + IMG_W * IMG_H * 2));
    ...
    region_layer_init(&detect_rl, 10, 7, 125, IMG_W, IMG_H);
    ...
    while (1)
    {
        ...
        if (g_save_flag)
        {
            ...
            sprintf(filePath, _T("0:photo%d.bmp"), fileNameCounter);
            
            //rgb565tobmp((uint8_t*)(g_ram_mux ? g_lcd_gram0 : g_lcd_gram1), 640, 480, filePath);
            rgb888_to_565(g_ai_buf, (g_ai_buf + IMG_W * IMG_H), (g_ai_buf + IMG_W * IMG_H * 2), (uint16_t *)g_display_buf, IMG_W * IMG_H);
            rgb565tobmp((uint8_t*)g_display_buf, IMG_W, IMG_H, filePath);
            //rgb565tobmp((uint8_t*)g_ai_buf, IMG_W, IMG_H, filePath);
            g_save_flag = 0;
